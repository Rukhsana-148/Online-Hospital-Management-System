...
if($name!=""&&$age!=0&&$dis!=""&&$con!=""&&$phn!=0&&$place!=""&&$room!=0&&$date!=""){
    $sql = "INSERT INTO `admit`(`Name`, `Age`, `Disease`, `Conditions`, `Address`, `Phone Number`, `Password`, `Room_No`, `Date`) VALUES ('$name','$age','$dis','$con','$place','$phn', '$pass','$room','$date')";
   $del = "DELETE FROM `patient` WHERE `Phone Number` = $phn";
   $del_room = "DELETE FROM `room` WHERE `Room_No` = $room";
    if ($conn->query($del) === TRUE && $conn->query($sql) === TRUE && $conn->query($del_room)===TRUE) {
      echo  "";
    }
 
    else {
    echo "Error: " . $del_room . "<br>" . $conn->error;
    }
    }
...
<body>
    ...
    <div class="row">
        ...
        <div class="row">
            <div class="col-sm-12">
                <h4>Approve is successful! <?php echo $_POST['name'];?></h4>
            </div>
</div>
</div></div>
</div>
